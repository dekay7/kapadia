...
│   ├── lib/
│   │   └── ip.js             ← shared SSRF-prevention helpers (parseIPv4, isPrivateIPv4/v6,
│   │                            isPrivateAddress); imported by link.js, osint.js, and chain.js
│   └── api/
...
│       └── chain.js          ← GET /api/chain — supply chain integrity auditor
├── scripts/
...
└── public/
    ...
    │   ├── index.css         ← homepage / terminal
    │   ├── tools.css         ← tools directory and shared tool chrome
    │   ├── docs.css          ← docs hub sidebar + reader
    │   ├── cv.css            ← Curriculum Vitae
    │   ├── writes.css        ← essays list + reader
    │   ├── about.css         ← about page
    │   ├── privacy.css       ← privacy policy page
    │   ├── 404.css           ← 404 error page
    │   ├── speed.css         ← Speed Test tool
    │   ├── link.css          ← Link Inspector tool
    │   ├── email.css         ← Email Analyzer tool
    │   ├── exif.css          ← EXIF Inspector tool
    │   ├── osint.css         ← OSINT Footprint tool
    │   └── chain.css         ← Supply Chain Auditor tool
    ...
    │   │   └── subnet.js
    │   └── vendor/
    │       ├── dompurify.min.js ← vendored DOMPurify 3.2.7 (SRI-pinned); used by docs + writes
    │       └── exifr.min.js     ← vendored exifr 7.1.3 (SRI-pinned); used by EXIF Inspector
    └── assets/
        ├── favicon.svg
        └── cv.pdf
```
